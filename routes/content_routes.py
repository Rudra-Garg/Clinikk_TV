...
@router.post("/content/", response_model=Content, summary="Create new content", description="Uploads a file and creates a content record")
...
@router.get("/content/", response_model=List[Content], summary="List contents", description="Retrieve a list of content records")
def list_contents(
    skip: int = 0,
    limit: int = 100,
    db: Session = Depends(get_db)
...
@router.get("/content/{content_id}", response_model=Content, summary="Get content", description="Retrieve a specific content record by ID")
...
@router.put("/content/{content_id}", response_model=Content, summary="Update content", description="Update a content record and optionally upload a new file")
...
@router.get("/content/{content_id}/stream", summary="Stream content", description="Generate a presigned URL for streaming content")
